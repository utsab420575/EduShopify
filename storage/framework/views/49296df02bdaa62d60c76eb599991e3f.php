...
<head>
    ...
    <title><?php echo e($title ?? 'Edushopify — B2B Education Procurement Marketplace'); ?></title>
    ...
    <header class="sticky top-0 z-50 bg-white/95 backdrop-blur border-b border-slate-200">
        <div class="max-w-[1400px] mx-auto px-4 lg:px-8 h-16 flex items-center justify-between gap-4">
            ...
            <a href="<?php echo e(url('/')); ?>" class="flex items-center gap-2 shrink-0">
                <img class="h-8 w-auto" src="<?php echo e(asset('images/logo.png')); ?>" alt="Edushopify Logo">
            </a>
            ...
            <div class="hidden md:flex items-center gap-6">
                ...
                <div class="flex space-x-6 items-center">
                    <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if(auth()->guard()->check()): ?>
                        <a href="<?php echo e(auth()->user()->isAdmin() ? url('/admin') : (auth()->user()->isSupplier() ? url('/supplier') : (auth()->user()->isBuyer() ? url('/buyer') : url('/dashboard')))); ?>" class="text-sm font-semibold text-slate-700 hover:text-brand-600 transition-colors font-display">Dashboard</a>
                    ...
                    <?php else: ?>
                        <a href="<?php echo e(route('login')); ?>" class="text-sm font-semibold text-slate-700 hover:text-brand-600 transition-colors font-display">Log in</a>
                        <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if(Route::has('register')): ?>
                            <a href="<?php echo e(route('register')); ?>" class="inline-flex items-center justify-center px-5 py-2.5 border border-transparent text-sm font-semibold rounded-md text-white bg-brand-600 hover:bg-brand-700 shadow-lg shadow-brand-500/30 transform hover:-translate-y-0.5 transition-all duration-200 font-display">
        ...
        <div id="mobile-menu" class="hidden md:hidden pb-4 px-4 space-y-3 bg-white border-b border-slate-200">
             <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if(auth()->guard()->check()): ?>
                <a href="<?php echo e(auth()->user()->isAdmin() ? url('/admin') : (auth()->user()->isSupplier() ? url('/supplier') : (auth()->user()->isBuyer() ? url('/buyer') : url('/dashboard')))); ?>" class="block px-3 py-2 text-base font-medium text-slate-700 hover:text-brand-600">Dashboard</a>
                <form method="POST" action="<?php echo e(route('logout')); ?>" class="block px-3 py-2">
                    <?php echo csrf_field(); ?>
                    <button type="submit" class="text-base font-medium text-slate-700 hover:text-brand-600 w-full text-left">Log out</button>
                </form>
            <?php else: ?>
                <a href="<?php echo e(route('login')); ?>" class="block px-3 py-2 text-base font-medium text-slate-700 hover:text-brand-600">Log in</a>
                <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if(Route::has('register')): ?>
                    <a href="<?php echo e(route('register')); ?>" class="block px-3 py-2 text-base font-medium text-brand-600 hover:text-brand-700">Reserve My Supplier Profile</a>
                <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
            <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
    ...
    <footer class="border-t border-slate-200 bg-white">
        <div class="max-w-[1400px] mx-auto px-4 lg:px-8 py-14">
            ...
            <div class="mt-10 pt-6 border-t border-slate-200 flex flex-col md:flex-row gap-6 items-center justify-between text-sm text-slate-500">
                <p>© <?php echo e(date('Y')); ?> Edushopify. All rights reserved.</p>
...
<?php /**PATH C:\laragon\www\edushopify\resources\views\components\layouts\app.blade.php ENDPATH**/ ?>