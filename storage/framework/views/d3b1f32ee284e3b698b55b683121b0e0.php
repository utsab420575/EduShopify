...
<?php $__env->startSection('body'); ?>
    ...
    <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-6">
        <div>
            ...
            <p class="text-sm text-gray-500 mt-1">Manage global system roles and custom company roles for your organization.</p>
        </div>
        <a href="<?php echo e(route('supplier.roles.create')); ?>" class="btn-primary text-sm font-semibold px-4 py-2 rounded-xl flex items-center gap-2">
            ...
            <div class="bg-white rounded-2xl border border-gray-200 p-6 flex flex-col justify-between hover:shadow-md transition-shadow">
                <div class="space-y-3">
                    ...
                    <p class="text-xs text-gray-600 line-clamp-2 min-h-[32px]">
                        <?php echo e($role->description ?: 'Standard role permissions for organization access.'); ?>
                ...
                <div class="pt-5 mt-4 border-t border-gray-100 flex items-center justify-between gap-2">
                    <div class="flex items-center gap-1">
                        <!-- Duplicate Role Form -->
                        <form method="POST" action="<?php echo e(route('supplier.roles.duplicate', $role)); ?>" onsubmit="return promptForNewRoleName(this, '<?php echo e(addslashes($role->display_name ?: $role->name)); ?>');" class="inline">
                            <?php echo csrf_field(); ?>
                        ...
                        <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if(!$isSystem): ?>
                            <a href="<?php echo e(route('supplier.roles.edit', $role)); ?>" title="Edit Custom Role" class="text-gray-500 hover:text-blue-600 p-2 rounded-lg hover:bg-gray-50 transition-colors">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>
                            </a>
                            <form method="POST" action="<?php echo e(route('supplier.roles.destroy', $role)); ?>" onsubmit="return confirm('Are you sure you want to delete this custom role?');" class="inline">
                                <?php echo csrf_field(); ?>
                    ...
                    <a href="<?php echo e(route('supplier.roles.show', $role)); ?>" class="btn-primary text-xs font-semibold px-3.5 py-1.5 rounded-lg flex items-center gap-1.5">
                        Manage &rarr;
                    </a>
                </div>
            </div>
    ...
    <script>
        function promptForNewRoleName(form, oldName) {
            const newName = prompt('Enter a name for the duplicated role:', oldName + ' (Copy)');
            if (newName && newName.trim().length > 0) {
                form.querySelector('input[name="new_display_name"]').value = newName.trim();
                return true;
            }
            return false;
        }
...
<?php echo $__env->make('backend.layouts.supplier', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\laragon\www\edushopify\resources\views\backend\supplier\access-control\roles\index.blade.php ENDPATH**/ ?>