...
<div id="sp-section-locations" class="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden" x-data="{ open: <?php echo e($spAccOpen('locations', false)); ?> }" x-init="$watch('open', v => localStorage.setItem('sp-acc-locations', v ? '1' : '0'))">
    <button @click="open = !open" type="button"
        ...
        <div class="flex items-center gap-3">
            ...
            <div class="text-left">
                ...
                <p class="text-xs text-gray-400"><?php echo e($serviceAreas->count() > 0 ? $serviceAreas->count().' area'.($serviceAreas->count() > 1 ? 's' : '') : 'Territories your company covers'); ?></p>
            </div>
        </div>
    ...
    <div x-show="open" x-cloak x-transition class="border-t border-gray-100 px-6 py-6">
        ...
        <?php echo $__env->make('backend.supplier.company.partials._section-errors', ['section' => 'locations'], array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?>

        <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php $__currentLoopData = $serviceAreas; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $area): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
            <div class="mb-4 border border-gray-200 rounded-xl overflow-hidden" x-data="{ editing: false }">
                <div class="flex items-center justify-between px-4 py-3 bg-gray-50">
                    <div class="flex items-center gap-2 min-w-0">
                        <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($area->is_primary): ?>
                        ...
                        <p class="text-sm font-semibold text-gray-900 truncate">
                            <?php echo e(collect([$area->country?->name, $area->state?->name, $area->city?->name])->filter()->implode(', ')); ?>

                        </p>
                        <p class="text-xs text-gray-500 hidden sm:block">
                            <?php echo e($area->radius_km ? 'Within '.$area->radius_km.' km' : ($area->city ? 'City-wide' : ($area->state ? 'State-wide' : 'Nationwide'))); ?>

                        </p>
                    </div>
                    <div class="flex items-center gap-2 flex-shrink-0 ml-2">
                        <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if (! ($area->is_primary)): ?>
                            <form method="POST" action="<?php echo e(route('supplier.company.profile.service-areas.primary', $area)); ?>">
                                <?php echo csrf_field(); ?>
                        ...
                        <form method="POST" action="<?php echo e(route('supplier.company.profile.service-areas.destroy', $area)); ?>" onsubmit="return confirmSwal(this, 'Remove this service area?', '', 'warning', 'Yes, remove')">
                ...
                <div x-show="editing" x-transition x-cloak class="p-4 border-t border-gray-100"
                     x-data="{
                         country: <?php echo e((int) $area->country_id); ?>,
                         state: <?php echo e((int) ($area->state_id ?? 0)); ?>,
                         states: <?php echo e(Js::from($area->area_states->map(fn($s) => ['id' => $s->id, 'name' => $s->name]))); ?>,
                         cities: <?php echo e(Js::from($area->area_cities->map(fn($c) => ['id' => $c->id, 'name' => $c->name]))); ?>,
                    ...
                    <form method="POST" action="<?php echo e(route('supplier.company.profile.service-areas.update', $area)); ?>">
                        <?php echo csrf_field(); ?>
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                            ...
                            <div>
                                ...
                                <select name="city_id" class="w-full text-sm rounded-lg border border-gray-300 px-3 py-2 bg-white focus:ring-2 focus:ring-emerald-300 outline-none">
                                    ...
                                    <template x-for="c in cities" :key="c.id"><option :value="c.id" x-text="c.name" :selected="c.id === <?php echo e((int) ($area->city_id ?? 0)); ?>"></option></template>
                                </select>
                            </div>
                        </div>
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-3">
                            <div>
                                ...
                                <input name="radius_km" value="<?php echo e($area->radius_km); ?>" type="number" class="w-full text-sm rounded-lg border border-gray-300 px-3 py-2 focus:ring-2 focus:ring-emerald-300 outline-none">
                            </div>
                            <label class="flex items-center gap-2 mt-6 text-sm text-gray-700 cursor-pointer">
                                <input name="is_primary" value="1" type="checkbox" <?php echo e($area->is_primary ? 'checked' : ''); ?> class="rounded" style="accent-color:var(--theme-primary)">
        ...
        <div class="mt-2" x-data="{ open: false }">
            ...
            <div x-show="open" x-transition x-cloak class="border border-emerald-200 bg-emerald-50/40 rounded-xl p-5"
                ...
                <form method="POST" action="<?php echo e(route('supplier.company.profile.service-areas.store')); ?>">
                    <?php echo csrf_field(); ?>
                    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                        <div>
                            <label class="block text-xs font-medium text-gray-600 mb-1">Country <span class="text-red-500">*</span></label>
...
<?php /**PATH C:\laragon\www\edushopify\resources\views\backend\supplier\company\partials\_locations.blade.php ENDPATH**/ ?>