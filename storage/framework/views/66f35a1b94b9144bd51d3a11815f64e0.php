...
<div id="sp-section-gallery" class="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden" x-data="{ open: <?php echo e($spAccOpen('gallery', false)); ?> }" x-init="$watch('open', v => localStorage.setItem('sp-acc-gallery', v ? '1' : '0'))">
    <button @click="open = !open" type="button"
        ...
        <div class="flex items-center gap-3">
            ...
            <div class="text-left">
                ...
                <p class="text-xs text-gray-400"><?php echo e($existingGallery->count()); ?> photo<?php echo e($existingGallery->count() === 1 ? '' : 's'); ?>, <?php echo e($videos->count()); ?> video<?php echo e($videos->count() === 1 ? '' : 's'); ?></p>
            </div>
        </div>
    ...
    <div x-show="open" x-cloak x-transition class="border-t border-gray-100 px-6 py-6">
        ...
        <?php echo $__env->make('backend.supplier.company.partials._section-errors', ['section' => 'gallery'], array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?>
            ...
            <div class="grid grid-cols-3 sm:grid-cols-5 lg:grid-cols-7 gap-2 mb-4">
                <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php $__currentLoopData = $existingGallery; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $img): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <div class="relative group aspect-square">
                        <img src="<?php echo e(asset('storage/'.$img->image_path)); ?>" class="w-full h-full object-cover rounded-xl border border-gray-200" alt="">
                        <form method="POST" action="<?php echo e(route('supplier.company.profile.gallery.destroy', $img)); ?>" onsubmit="return confirmSwal(this, 'Remove this image?', '', 'warning', 'Yes, remove')" class="absolute top-1 right-1">
        ...
        <form method="POST" action="<?php echo e(route('supplier.company.profile.gallery.store')); ?>" enctype="multipart/form-data"
              x-data="{ previews: [] }">
            <?php echo csrf_field(); ?>
...
unset($__errorArgs, $__bag); ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
        ...
        <div class="mt-6 pt-6 border-t border-gray-100">
                    ...
                    <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php $__currentLoopData = $videos; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $video): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <div class="p-3 border border-gray-200 rounded-xl flex items-center justify-between gap-2">
                            <div class="flex items-center gap-3 min-w-0">
                                ...
                                <div class="min-w-0">
                                    <p class="text-sm font-semibold text-gray-900 truncate"><?php echo e($video->title); ?></p>
                                    <a href="<?php echo e($video->video_url); ?>" target="_blank" class="text-xs text-indigo-600 hover:underline truncate block"><?php echo e($video->video_url); ?></a>
                                </div>
                            </div>
                            <form method="POST" action="<?php echo e(route('supplier.company.profile.videos.destroy', $video)); ?>" onsubmit="return confirmSwal(this, 'Remove this video?', '', 'warning', 'Yes, remove')">
                                <?php echo csrf_field(); ?> <?php echo method_field('DELETE'); ?>
            ...
            <form method="POST" action="<?php echo e(route('supplier.company.profile.videos.store')); ?>" class="grid grid-cols-1 sm:grid-cols-3 gap-3 items-end">
                <?php echo csrf_field(); ?>
                <div>
                    <label class="block text-xs font-medium text-gray-600 mb-1">Video Title</label>
                    <input name="title" value="<?php echo e(old('title')); ?>" type="text" placeholder="e.g. Factory Tour" class="w-full text-sm rounded-lg border border-gray-300 px-3 py-2 focus:ring-2 focus:ring-pink-300 outline-none">
...
unset($__errorArgs, $__bag); ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
                ...
                <div>
                    ...
                    <input name="video_url" value="<?php echo e(old('video_url')); ?>" type="url" placeholder="https://www.youtube.com/watch?v=..." class="w-full text-sm rounded-lg border border-gray-300 px-3 py-2 focus:ring-2 focus:ring-pink-300 outline-none">
...
<?php /**PATH C:\laragon\www\edushopify\resources\views\backend\supplier\company\partials\_gallery.blade.php ENDPATH**/ ?>