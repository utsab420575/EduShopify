{{-- Main Content: Category Tabs + Blog Articles Grid + Live Search --}}
<style>
...
<div class="max-w-7xl mx-auto px-4 py-8">
  ...
  <div class="overflow-x-auto pb-3 mb-6 -mx-4 px-4 scrollbar-none">
    <div class="flex items-center gap-2 min-w-max" id="blog-tabs-container">
      ...
      @foreach($categories as $category)
        <button
          type="button"
          class="category-tab-btn {{ $activeCategory === $category->slug ? 'active' : '' }}"
          data-category="{{ $category->slug }}"
          data-name="{{ $category->name }}"
        >
          @if($category->icon)
            <i class="{{ $category->icon }} mr-1.5 text-xs opacity-80"></i>
          @endif
  ...
  <div class="flex items-center justify-between mb-6 text-sm text-gray-500">
    <div id="blog-results-summary">
      Showing <span id="blog-results-count" class="font-bold text-gray-900">{{ $articles->total() }}</span> articles
      <span id="blog-active-cat-label" class="font-medium text-emerald-600">
        {{ $activeCategory !== 'all' ? 'in ' . ($categories->firstWhere('slug', $activeCategory)?->name ?? 'Category') : '' }}
      </span>
      @if($search)
        matching "<span id="blog-active-search-label" class="text-gray-900 font-semibold">{{ $search }}</span>"
      @endif
    </div>
  </div>
  ...
  <div
    id="blog-grid"
    class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 mb-10 {{ $articles->isEmpty() ? 'hidden' : '' }}"
  >
    @foreach($articles as $article)
      ...
      <a href="{{ route('v2.blogs.show', $article->slug) }}"
         ...
         style="animation-delay: {{ $loop->index * 45 }}ms">
        ...
        <div class="p-5 flex-1 flex flex-col justify-between">
          <div>
            ...
            <div class="flex items-center gap-2 mb-2.5 text-[11px] text-gray-500">
              ...
              <span class="font-medium text-gray-700 truncate max-w-[130px]">{{ $authorName }}</span>
              <span class="text-gray-300">•</span>
              <span class="text-gray-400 whitespace-nowrap">{{ $article->published_at?->format('M d, Y') }}</span>
            </div>
          ...
          {{-- Bottom Row: Read Time, Likes, Comments, Action --}}
          <div class="pt-3 border-t border-gray-100 flex items-center justify-between mt-auto">
            <div class="flex items-center gap-3 text-[11px] text-gray-400">
              <span class="flex items-center gap-1">
                ...
                {{ $article->reading_time_minutes ?? 5 }}m read
              </span>
  ...
  <div
    id="blog-empty-state"
    class="text-center py-16 px-4 bg-white rounded-2xl border border-gray-200 {{ $articles->isEmpty() ? '' : 'hidden' }}"
  >
  ...
  {{-- Server Pagination --}}
  <div id="blog-pagination" class="mt-8 {{ $articles->hasPages() ? '' : 'hidden' }}">
    {{ $articles->links() }}
  </div>
</div>

{{-- ── Client-Side Interactive Live Filter & AJAX Search ── --}}
<script>
document.addEventListener('DOMContentLoaded', function () {
  ...
  function fetchArticles() {
    ...
    const url = '{{ route("v2.blogs.index") }}?' + params.toString();
