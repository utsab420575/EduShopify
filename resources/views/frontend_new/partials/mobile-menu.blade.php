...
<div id="fn-mobile-menu" class="fixed top-0 left-0 h-full w-72 bg-white z-50 shadow-xl transform -translate-x-full transition-transform duration-200 lg:hidden flex flex-col">
  <div class="flex items-center justify-between px-4 h-14 border-b border-gray-200">
    <a href="{{ route('v2.home') }}" class="flex items-center">
      <img src="{{ asset('storage/System_Files/Logo/edushopifyLogo.png') }}" alt="Edushopify" class="h-8 w-auto object-contain" />
    </a>
  ...
  <nav class="flex flex-col px-2 py-3 gap-1 flex-1 overflow-y-auto">
    <a href="{{ route('v2.products.index') }}" class="px-3 py-2.5 rounded-md text-sm font-medium {{ request()->routeIs('v2.categories.*', 'v2.products.*') ? 'text-gray-900 bg-gray-100' : 'text-gray-700 hover:bg-gray-50' }}">Products</a>
    ...
    <a href="{{ route('v2.blogs.index') }}" class="px-3 py-2.5 rounded-md text-sm font-medium {{ request()->routeIs('v2.blogs.*') ? 'text-gray-900 bg-gray-100' : 'text-gray-700 hover:bg-gray-50' }}">Blog</a>
    ...
    <a href="{{ route('v2.compare.index') }}" class="flex items-center justify-between px-3 py-2.5 rounded-md text-sm font-medium {{ request()->routeIs('v2.compare.*') ? 'text-gray-900 bg-gray-100' : 'text-gray-700 hover:bg-gray-50' }}">
  ...
  <div class="px-4 py-4 border-t border-gray-100 space-y-2">
    @guest
      <a href="{{ route('login', ['redirect' => url()->current()]) }}" class="block text-center py-2.5 text-sm font-medium text-gray-700">Sign In</a>
      <a href="{{ route('register', ['redirect' => url()->current()]) }}" class="block text-center bg-emerald-500 hover:bg-emerald-600 text-white text-sm font-semibold py-2.5 rounded-lg">Register</a>
    @else
