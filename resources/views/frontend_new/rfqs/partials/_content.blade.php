...
{{-- 1. Skeleton Loading State (Visible immediately on page refresh) --}}
@include('frontend_new.rfqs.partials._skeleton')

{{-- 2. Real RFQ Cards List (Reveals with Staggered Fade-Up Animation once data is ready) --}}
<div class="flex flex-col gap-4" id="rfq-list" style="display: none;">
  @forelse($opportunities as $rfq)
    @include('frontend_new.rfqs.partials._card', ['rfq' => $rfq])
  @empty
...
{{-- Empty state for live search (hidden unless search yields no matches) --}}
  ...
  @if($opportunities->hasPages())
    <div class="flex items-center justify-between gap-4 mt-8">
      <div>
        ...
        @else
          <a href="{{ $opportunities->previousPageUrl() }}" class="px-3.5 py-2 rounded-md text-sm font-medium text-gray-600 border border-gray-300 hover:bg-gray-50 transition-colors">Previous</a>
        @endif
      </div>
      <p class="text-sm text-gray-500">Page {{ $opportunities->currentPage() }} of {{ $opportunities->lastPage() }}</p>
      <div>
        @if($opportunities->hasMorePages())
          <a href="{{ $opportunities->nextPageUrl() }}" class="px-3.5 py-2 rounded-md text-sm font-medium text-gray-600 border border-gray-300 hover:bg-gray-50 transition-colors">Next</a>
        @else
