...
<header class="h-20 shrink-0 bg-white border-b flex items-center justify-between px-4 lg:px-6" style="border-color:var(--topbar-border)">
    <div class="flex items-center gap-3 min-w-0">
        ...
        <div class="hidden sm:block min-w-0">
            <p class="text-xs text-gray-400 truncate">
                <a href="{{ route('buyer.dashboard') }}" class="hover:text-gray-600">Buyer Dashboard</a>
    ...
    <div class="flex items-center gap-2">
        @if($bothCapabilities)
            <a href="{{ route('supplier.dashboard') }}" class="hidden sm:inline-flex items-center gap-1.5 text-xs font-medium px-3 py-2 rounded-lg border border-gray-300 text-gray-600 hover:bg-gray-50">
        ...
        <div class="relative">
            ...
            <div x-show="notifOpen" @click.outside="notifOpen=false" x-transition x-cloak
                ...
                <div class="px-4 py-3 border-b border-gray-100 flex items-center justify-between">
                    ...
                    <a href="{{ route('buyer.notifications.index') }}" class="text-xs font-medium" style="color:var(--theme-primary)">View all</a>
                </div>
                <div class="max-h-72 overflow-y-auto divide-y divide-gray-100">
                    @forelse(($topbarNotifications ?? []) as $notification)
                        <a href="{{ route('buyer.notifications.index') }}" class="flex gap-3 px-4 py-3 hover:bg-gray-50">
                ...
                <a href="{{ route('buyer.notifications.index') }}" class="block text-center text-xs font-medium py-2.5 border-t border-gray-100" style="color:var(--theme-primary)">View all notifications</a>
            </div>
        </div>
        ...
        <div class="relative">
            ...
            <div x-show="profileOpen" @click.outside="profileOpen=false" x-transition x-cloak
                ...
                <a href="{{ route('buyer.profile.edit') }}" class="flex items-center gap-2 px-4 py-2 text-sm text-gray-700 hover:bg-gray-50"><i class="fa-regular fa-user w-4 text-gray-400"></i>My Profile</a>
                <a href="{{ route('buyer.settings.security') }}" class="flex items-center gap-2 px-4 py-2 text-sm text-gray-700 hover:bg-gray-50"><i class="fa-solid fa-gear w-4 text-gray-400"></i>Settings</a>
