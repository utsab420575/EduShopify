...
@section('breadcrumb', 'Access Control / Roles / Create')
...
@section('body')
    ...
    <div class="max-w-5xl mx-auto space-y-6">
        <div class="flex items-center justify-between">
            ...
            <a href="{{ route('supplier.roles.index') }}" class="text-xs font-semibold text-gray-600 hover:text-gray-900 px-3.5 py-2 bg-white border border-gray-300 rounded-xl">
                &larr; Back to Roles
            </a>
        </div>
        ...
        <form method="POST" action="{{ route('supplier.roles.store') }}" class="space-y-6">
            @csrf
            ...
            <div class="bg-white rounded-2xl border border-gray-200 p-6 space-y-4">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="text" name="display_name" value="{{ old('display_name') }}" required placeholder="e.g. Senior Catalog Specialist" class="w-full px-4 py-2.5 rounded-xl border border-gray-300 text-sm focus:ring-2 focus:ring-indigo-500 focus:outline-none">
                    </div>
                    <div>
                        <label class="block text-xs font-bold text-gray-700 uppercase tracking-wider mb-1">Description</label>
                        <input type="text" name="description" value="{{ old('description') }}" placeholder="Role responsibilities in your team" class="w-full px-4 py-2.5 rounded-xl border border-gray-300 text-sm focus:ring-2 focus:ring-indigo-500 focus:outline-none">
                    </div>
                </div>
            </div>
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                                @foreach($permissions as $perm)
                                    <label class="flex items-start gap-2.5 p-2.5 bg-white rounded-lg border border-gray-200 hover:border-indigo-300 hover:bg-indigo-50/20 cursor-pointer transition-colors">
                                        <input type="checkbox" name="permissions[]" value="{{ $perm->name }}" {{ in_array($perm->name, old('permissions', [])) ? 'checked' : '' }} class="perm-checkbox w-4 h-4 mt-0.5 rounded text-indigo-600 focus:ring-indigo-500 border-gray-300">
                                        <div class="text-xs">
            ...
            <div class="flex items-center justify-end gap-3 pt-4">
                <a href="{{ route('supplier.roles.index') }}" class="px-5 py-2.5 text-sm font-semibold text-gray-700 bg-white border border-gray-300 rounded-xl hover:bg-gray-50">
                    Cancel
                </a>
