{{--
    Reusable service-icon picker. Include with:
      @include('backend.supplier.company.partials._icon-picker', ['selectedIconId' => $service->icon_id])
    Must be included inside an element whose Alpine scope already has
    `iconCatalog` defined (see _services.blade.php's outer x-data).
--}}
<div class="mt-3 relative" x-data="{
    openPicker: false,
    search: '',
    selectedId: {{ $selectedIconId ?? 'null' }},
    ...
    {{-- Dropdown Popover with high z-index --}}
