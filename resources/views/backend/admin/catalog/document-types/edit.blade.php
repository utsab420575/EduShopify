...
@section('title', 'Edit ' . $documentType->name)
@section('breadcrumb', 'Catalog & Taxonomy / Document Types / Edit')

@section('body')

    <x-backend.page-header :title="'Edit ' . $documentType->name" />

    <form method="POST" action="{{ route('admin.catalog.document-types.update', $documentType) }}" class="space-y-6">
        @csrf @method('PUT')
