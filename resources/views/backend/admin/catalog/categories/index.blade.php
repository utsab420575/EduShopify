        ...
        @else
            <x-slot:head>
                <tr>
                    ...
                    <th class="px-5 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wider">Children</th>
            ...
            @foreach($categories as $category)
                <tr class="hover:bg-gray-50">
                    <td class="px-5 py-3.5 text-sm font-medium text-gray-900">{{ $category->name }}</td>
                    <td class="px-5 py-3.5 text-sm text-gray-600">{{ $category->parent?->name ?? '—' }}</td>
                    <td class="px-5 py-3.5 text-sm text-gray-600">{{ ucfirst($category->type) }}</td>
                    <td class="px-5 py-3.5 text-sm text-gray-600">{{ $category->children_count }}</td>
                    <td class="px-5 py-3.5">
                        <x-backend.status-badge :status="$category->is_active ? 'active' : 'inactive'" />
                    </td>
                    <td class="px-5 py-3.5 text-right">
                        <div class="flex items-center justify-end gap-1.5">
                            <a href="{{ route('admin.catalog.categories.edit', $category) }}" class="w-8 h-8 rounded-lg inline-flex items-center justify-center text-gray-500 hover:bg-gray-100"><i class="fa-regular fa-pen-to-square"></i></a>
