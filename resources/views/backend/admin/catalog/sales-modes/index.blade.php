...
@section('title', 'Sales Modes')
@section('breadcrumb', 'Catalog & Taxonomy / Sales Modes')
...
<div x-data="{
    ...
    openCreate() {
        this.isEdit = false;
        this.formAction = '{{ route('admin.catalog.sales-modes.store') }}';
        this.form = { id: null, name: '', code: '', description: '', sort_order: 0, is_active: true };
        this.modalOpen = true;
    },
    openEdit(item) {
        this.isEdit = true;
        this.formAction = '{{ url('admin/catalog/sales-modes') }}/' + item.id;
    ...
    <x-backend.page-header title="Sales Modes" subtitle="Define how buyers can transact on listings — RFQ only, direct purchase, or both.">
        <x-slot:actions>
    ...
    {{-- Toolbar --}}
    <div class="mb-4">
        <form method="GET" action="{{ route('admin.catalog.sales-modes.index') }}" class="flex flex-wrap items-center gap-3">
            ...
            @if($search || $status)
                <a href="{{ route('admin.catalog.sales-modes.index') }}" class="text-xs text-gray-500 hover:text-gray-800">Clear</a>
            @endif
    ...
    <x-backend.table>
        @if($salesModes->isEmpty())
            <x-slot:empty><x-backend.empty-state icon="fa-handshake" title="No sales modes found" /></x-slot:empty>
        @else
            <x-slot:head>
                <tr>
            ...
            @foreach($salesModes as $item)
            <tr class="hover:bg-gray-50/80 transition-colors">
                <td class="px-5 py-3.5 text-sm font-semibold text-gray-900">{{ $item->name }}</td>
                <td class="px-5 py-3.5">
                    <span class="inline-flex items-center font-mono text-xs font-bold px-2 py-0.5 rounded bg-teal-50 text-teal-700 border border-teal-100">{{ $item->code }}</span>
                </td>
                ...
                <td class="px-5 py-3.5 text-right">
                    <div class="flex items-center justify-end gap-1.5">
                        @can('platform.attributes.manage')
                        <form method="POST" action="{{ route('admin.catalog.sales-modes.toggle-active', $item) }}"
                              onsubmit="return confirmSwal(this, '{{ $item->is_active ? 'Deactivate' : 'Activate' }} Sales Mode?', '{{ addslashes($item->name) }} will be {{ $item->is_active ? 'hidden from suppliers' : 'made available' }}.', 'question', 'Yes, {{ $item->is_active ? 'Deactivate' : 'Activate' }}')">
                            @csrf
                            <button type="submit"
                        ...
                        <form method="POST" action="{{ route('admin.catalog.sales-modes.destroy', $item) }}"
                              onsubmit="return confirmSwal(this, 'Delete Sales Mode?', 'Delete {{ addslashes($item->name) }}? This cannot be undone.', 'warning', 'Yes, Delete')">
                            @csrf @method('DELETE')
            ...
            <x-slot:pagination>
                <x-backend.pagination :paginator="$salesModes" />
            </x-slot:pagination>
        @endif
    </x-backend.table>
