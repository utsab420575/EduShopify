...
@section('body')
    ...
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-6" x-data="{
        ...
        <div class="lg:col-span-4 space-y-4">
            ...
            <div class="bg-white rounded-xl border border-gray-200 p-5 text-center">
                ...
                <a href="{{ route('admin.catalog.attributes.import.template') }}" class="block mt-2 text-xs text-gray-400 hover:text-gray-600">
            ...
            <div class="bg-white rounded-xl border border-gray-200 p-5 text-center">
                ...
                <a href="{{ route('admin.catalog.attributes.values-import.template') }}" class="block mt-2 text-xs text-gray-400 hover:text-gray-600">
                    <i class="fa-solid fa-download mr-1"></i> Download template
                </a>
            </div>
        </div>
    ...
    {{-- IMPORT MODAL — upload a CSV of attributes --}}
    <div x-data="{ open: {{ session('open_attribute_import') ? 'true' : 'false' }} }" @open-import-attributes.window="open = true"
         x-show="open" x-cloak class="fixed inset-0 z-50 flex items-center justify-center p-4" style="display:none;">
        <div class="fixed inset-0 bg-gray-900/50 backdrop-blur-sm" @click="open = false"></div>
        <div class="relative bg-white rounded-2xl shadow-2xl max-w-lg w-full p-6 border border-gray-100"
            ...
            <div class="pt-4 space-y-4">
                ...
                <form method="POST" action="{{ route('admin.catalog.attributes.import.preview') }}" enctype="multipart/form-data" class="space-y-4">
                    @csrf
                    <div>
                    ...
                    <div class="flex items-center justify-between gap-2 pt-3 border-t border-gray-100">
                        <a href="{{ route('admin.catalog.attributes.import.template') }}" class="text-xs font-medium text-indigo-600 hover:underline">
                            <i class="fa-solid fa-download mr-1"></i> Download template
                        </a>
    ...
    {{-- IMPORT PREVIEW MODAL — attributes --}}
    @if(session('attribute_import_preview'))
        @php($attributeImportPreview = session('attribute_import_preview'))
        <div x-data="{ open: {{ session('open_attribute_import_preview') ? 'true' : 'false' }} }"
             x-show="open" x-cloak class="fixed inset-0 z-50 flex items-center justify-center p-4" style="display:none;">
            <div class="fixed inset-0 bg-gray-900/50 backdrop-blur-sm" @click="open = false"></div>
            <div class="relative bg-white rounded-2xl shadow-2xl max-w-2xl w-full p-6 border border-gray-100 max-h-[88vh] flex flex-col"
                ...
                <div class="py-4 flex items-center gap-4 shrink-0">
                    <span class="inline-flex items-center gap-1.5 text-xs font-semibold px-3 py-1.5 rounded-full bg-emerald-50 text-emerald-700 border border-emerald-200">
                        <i class="fa-solid fa-plus"></i> {{ $attributeImportPreview['created_count'] }} new
                    </span>
                    <span class="inline-flex items-center gap-1.5 text-xs font-semibold px-3 py-1.5 rounded-full bg-gray-50 text-gray-600 border border-gray-200">
                        <i class="fa-solid fa-check"></i> {{ $attributeImportPreview['existing_count'] }} already exist (will be reused)
                    </span>
                    @if($attributeImportPreview['error_count'] > 0)
                        <span class="inline-flex items-center gap-1.5 text-xs font-semibold px-3 py-1.5 rounded-full bg-red-50 text-red-700 border border-red-200">
                            <i class="fa-solid fa-triangle-exclamation"></i> {{ $attributeImportPreview['error_count'] }} row error{{ $attributeImportPreview['error_count'] === 1 ? '' : 's' }}
                        </span>
                    @endif
                </div>

                <div class="flex-1 overflow-y-auto space-y-2 pr-1">
                    @foreach($attributeImportPreview['rows'] as $row)
                            ...
                            @else
                                <p class="text-xs flex flex-wrap items-center gap-2">
                                    ...
                                    @if(isset($row['input_type']))
                                        <span class="text-[10px] text-gray-400 uppercase font-semibold">{{ str_replace('_', ' ', $row['input_type']) }}</span>
                                    @endif
                ...
                <div class="flex items-center justify-end gap-2 pt-4 mt-2 border-t border-gray-100 shrink-0">
                    <button type="button" @click="open = false" class="px-4 py-2 text-xs font-medium border border-gray-300 rounded-lg text-gray-700 hover:bg-gray-50">Cancel</button>
                    <form method="POST" action="{{ route('admin.catalog.attributes.import.store') }}">
                        @csrf
    ...
    {{-- IMPORT MODAL — upload a CSV of attribute values --}}
    <div x-data="{ open: {{ session('open_attribute_value_import') ? 'true' : 'false' }} }" @open-import-attribute-values.window="open = true"
         x-show="open" x-cloak class="fixed inset-0 z-50 flex items-center justify-center p-4" style="display:none;">
        <div class="fixed inset-0 bg-gray-900/50 backdrop-blur-sm" @click="open = false"></div>
        <div class="relative bg-white rounded-2xl shadow-2xl max-w-lg w-full p-6 border border-gray-100"
             x-show="open" x-transition:enter="transition ease-out duration-150" x-transition:enter-start="opacity-0 scale-95" x-transition:enter-end="opacity-100 scale-100">
            ...
            <div class="pt-4 space-y-4">
                ...
                <form method="POST" action="{{ route('admin.catalog.attributes.values-import.preview') }}" enctype="multipart/form-data" class="space-y-4">
                    @csrf
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1.5">CSV File</label>
                        <input type="file" name="file" accept=".csv,text/csv" required
                    ...
                    <div class="flex items-center justify-between gap-2 pt-3 border-t border-gray-100">
                        <a href="{{ route('admin.catalog.attributes.values-import.template') }}" class="text-xs font-medium text-indigo-600 hover:underline">
                            <i class="fa-solid fa-download mr-1"></i> Download template
                        </a>
                        <div class="flex items-center gap-2">
                            <button type="button" @click="open = false" class="px-4 py-2 text-xs font-medium border border-gray-300 rounded-lg text-gray-700 hover:bg-gray-50">Cancel</button>
    ...
    {{-- IMPORT PREVIEW MODAL — attribute values --}}
    @if(session('attribute_value_import_preview'))
        @php($attributeValueImportPreview = session('attribute_value_import_preview'))
        <div x-data="{ open: {{ session('open_attribute_value_import_preview') ? 'true' : 'false' }} }"
             x-show="open" x-cloak class="fixed inset-0 z-50 flex items-center justify-center p-4" style="display:none;">
            <div class="fixed inset-0 bg-gray-900/50 backdrop-blur-sm" @click="open = false"></div>
            <div class="relative bg-white rounded-2xl shadow-2xl max-w-2xl w-full p-6 border border-gray-100 max-h-[88vh] flex flex-col"
                 x-show="open" x-transition:enter="transition ease-out duration-150" x-transition:enter-start="opacity-0 scale-95" x-transition:enter-end="opacity-100 scale-100">
                ...
                <div class="py-4 flex items-center gap-4 shrink-0">
                    <span class="inline-flex items-center gap-1.5 text-xs font-semibold px-3 py-1.5 rounded-full bg-emerald-50 text-emerald-700 border border-emerald-200">
                        <i class="fa-solid fa-plus"></i> {{ $attributeValueImportPreview['created_count'] }} new
                    </span>
                    <span class="inline-flex items-center gap-1.5 text-xs font-semibold px-3 py-1.5 rounded-full bg-gray-50 text-gray-600 border border-gray-200">
                        <i class="fa-solid fa-check"></i> {{ $attributeValueImportPreview['existing_count'] }} already exist (will be reused)
                    </span>
                    @if($attributeValueImportPreview['error_count'] > 0)
                        <span class="inline-flex items-center gap-1.5 text-xs font-semibold px-3 py-1.5 rounded-full bg-red-50 text-red-700 border border-red-200">
                            <i class="fa-solid fa-triangle-exclamation"></i> {{ $attributeValueImportPreview['error_count'] }} row error{{ $attributeValueImportPreview['error_count'] === 1 ? '' : 's' }}
                        </span>
                    @endif
                </div>

                <div class="flex-1 overflow-y-auto space-y-2 pr-1">
                    @foreach($attributeValueImportPreview['rows'] as $row)
                        <div class="border border-gray-100 rounded-lg px-3 py-2">
                            @if(isset($row['error']))
                                <p class="text-xs text-red-600"><i class="fa-solid fa-triangle-exclamation mr-1"></i>Line {{ $row['line'] }}: {{ $row['attribute_name'] }} — {{ $row['value'] }} — {{ $row['error'] }}</p>
                            @else
                                <p class="text-xs flex flex-wrap items-center gap-2">
                                    <span class="text-gray-400">{{ $row['attribute_name'] }}</span>
                                    <i class="fa-solid fa-chevron-right text-[8px] text-gray-300"></i>
                                    <span class="{{ $row['status'] === 'existing' ? 'text-gray-500' : 'text-emerald-700 font-semibold' }}">{{ $row['value'] }}</span>
                                    <span class="text-[10px] font-semibold px-1.5 py-0.5 rounded-full {{ $row['status'] === 'would_create' ? 'bg-emerald-50 text-emerald-700' : 'bg-gray-50 text-gray-500' }}">
                                        {{ $row['status'] === 'would_create' ? 'Creates new' : 'Already exists' }}
                                    </span>
                                </p>
                ...
                <div class="flex items-center justify-end gap-2 pt-4 mt-2 border-t border-gray-100 shrink-0">
                    <button type="button" @click="open = false" class="px-4 py-2 text-xs font-medium border border-gray-300 rounded-lg text-gray-700 hover:bg-gray-50">Cancel</button>
                    <form method="POST" action="{{ route('admin.catalog.attributes.values-import.store') }}">
                        @csrf
                        <button type="submit" class="btn-primary text-xs font-semibold px-4 py-2 rounded-lg">
                            <i class="fa-solid fa-check mr-1"></i> Confirm Import
                        </button>
