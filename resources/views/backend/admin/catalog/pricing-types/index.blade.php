...
@section('title', 'Pricing Types')
@section('breadcrumb', 'Catalog & Taxonomy / Pricing Types')
...
<div x-data="{
    ...
    openCreate() {
        this.isEdit = false;
        this.formAction = '{{ route('admin.catalog.pricing-types.store') }}';
        this.form = { id: null, name: '', code: '', description: '', sort_order: 0, is_active: true };
        this.modalOpen = true;
    },
    openEdit(item) {
        this.isEdit = true;
        this.formAction = '{{ url('admin/catalog/pricing-types') }}/' + item.id;
    ...
    <x-backend.page-header title="Pricing Types" subtitle="Define the pricing models available for supplier listings (e.g. Fixed Price, Quote Only, RFQ Enabled).">
        <x-slot:actions>
    ...
    <div class="mb-4">
        <form method="GET" action="{{ route('admin.catalog.pricing-types.index') }}" class="flex flex-wrap items-center gap-3">
            ...
            @if($search || $status)
                <a href="{{ route('admin.catalog.pricing-types.index') }}" class="text-xs text-gray-500 hover:text-gray-800">Clear</a>
            @endif
    ...
    <x-backend.table>
        @if($pricingTypes->isEmpty())
            <x-slot:empty><x-backend.empty-state icon="fa-tag" title="No pricing types found" /></x-slot:empty>
        @else
            <x-slot:head>
                <tr>
            ...
            <tr class="hover:bg-gray-50/80 transition-colors">
                ...
                <td class="px-5 py-3.5 text-right">
                    <div class="flex items-center justify-end gap-1.5">
                        {{-- Toggle Active --}}
                        @can('platform.attributes.manage')
                        <form method="POST" action="{{ route('admin.catalog.pricing-types.toggle-active', $item) }}"
                              onsubmit="return confirmSwal(this, '{{ $item->is_active ? 'Deactivate' : 'Activate' }} Pricing Type?', '{{ addslashes($item->name) }} will be {{ $item->is_active ? 'hidden from suppliers' : 'available to suppliers' }}.', 'question', 'Yes, {{ $item->is_active ? 'Deactivate' : 'Activate' }}')">
                            @csrf
                            <button type="submit"
                        ...
                        {{-- Delete --}}
                        <form method="POST" action="{{ route('admin.catalog.pricing-types.destroy', $item) }}"
                              onsubmit="return confirmSwal(this, 'Delete Pricing Type?', 'Delete {{ addslashes($item->name) }}? This cannot be undone.', 'warning', 'Yes, Delete')">
                            @csrf @method('DELETE')
                            <button type="submit"
            ...
            <x-slot:pagination>
                <x-backend.pagination :paginator="$pricingTypes" />
            </x-slot:pagination>
        @endif
    </x-backend.table>
