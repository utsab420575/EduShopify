{{--
    Dynamic category-attribute inputs for one RFQ item — adapted from
    supplier/catalog/listings/partials/attributes-form.blade.php's per-
    input-type templates. Included inside _item.blade.php's x-for scope, so
    `item` and `index` are already in context (the outer items x-for).
    Writes into item.attribute_values[attr.id], submitted as
    items[index][attribute_values][attr.id][...] — synced server-side into
    rfq_item_attribute_values by RfqService::syncItemAttributeValues().
--}}
