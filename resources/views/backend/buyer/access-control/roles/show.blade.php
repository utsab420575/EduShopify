...
@section('body')
    ...
    <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-6">
        <div>
            ...
            <p class="text-sm text-gray-500 mt-1">{{ $role->description ?: 'Role permissions and member assignments for this institution.' }}</p>
        </div>

        <div class="flex items-center gap-2">
            <form method="POST" action="{{ route('buyer.roles.duplicate', $role) }}" onsubmit="const name = prompt('Name for the duplicated role:', '{{ addslashes($role->display_name ?: $role->name) }} (Copy)'); if (name) { this.querySelector('input[name=new_display_name]').value = name; return true; } return false;">
                @csrf
            ...
            @if(!$role->is_system && $role->account_id === $account->id)
                <a href="{{ route('buyer.roles.edit', $role) }}" class="px-3.5 py-2 text-xs font-semibold text-white bg-blue-600 rounded-xl hover:bg-blue-700 flex items-center gap-1.5">
